        ...
        <section id="features" className="py-24 relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                >
                    ...
                    <p className="text-xl text-muted-foreground max-w-3xl mx-auto">Tranform your photo with cutting edge
                        AI technology. Each features is design to give you professional result in second, not hours.
                    </p>
                </motion.div>
                ...
                <div className="grid lg:grid-cols-2 gap-8 max-w-4xl mx-auto">
                    {features.slice(3).map((features, index) => (
                        <FeatureCard key={features.title} feature={features} index={index + 3}/>
                    ))}
                </div>
            </div>
        </section>
...
function FeatureCard({feature, index}: { feature: any; index: number }) {
    ...
    return (
        ...
        >
            <div
                className="h-full group glass rounded-2xl p-8 border border-card-border hover:border-primary/30 transition-all duration-300 shadow-glow-subtle hover:shadow-glow-primary">
                <div className="relative mb-6">
                    ...
                    <div
                        className="absolute inset-0 w-16 h-16 rounded-2xl bg-gradient-to-br from-primary/20 to-secondary/20 blur-xl group-hover:blur-2xl transition-all duration-300"/>
                </div>
                <h3 className="text-2xl font-bold mb-4 text-foreground group-hover:text-primary transition-col  ors">{title}</h3>
                <p className="text-muted-foreground leading-relaxed">{description}</p>
                <div className="mt-6 pt-6 border-t border-card-border">
                    <div className="flex items-center space-x-2 text-sm text-primary">
                        <div className="w-2 h-2 bg-primary rounded-full animate-pulse"/>
                        <span className="font-medium">OzuPicai Super AI Power</span>
                    </div>
                </div>
            </div>
        </motion.div>
